...
$(document).ready(function() {
	...
	function productAdd(){
		...
		text += "<td class='prod'><span><input type='text' id='pdt_name_"+num+"' name='pdt_name' placeholder=\"품목\"></span></td>";
		text += "<td class='prod'><span><input type='text' id='pdt_standard_"+num+"' name='pdt_standard' placeholder=\"규격\"></span></td>";
		...
		if (type=="view"){ //상세보기
			...
			$.ajax({
	            ...
	            success : function(data) {
					...
					var contract_phonenumber = "";
					var supply_amount = "";
					var contract_manager_name = ""
					var buyer_manager_name = "";
					var buyer_phonenumber = "";
					var post_no ="";
					var buyer_addr ="";
					var buyer_addr_info ="";
					var pdt_cd = "";
	            	...
	            	if (array.length > 0) {
						...
						buyer_addr_info = array[0].buyer_addr_info;
						...
						$('#buyer_addr_info').val(buyer_addr_info);
	            		...
	            		$('#cancel_reason').val(cancel_reason);		
	                   	...
	                   	for (var i in array){
							...
							text += "<tr id='product_"+no+"'>";
							text += "<td class='num pctnum'><a href='#' onclick='productDelete(\""+no+"\")'><img src='/images/common/miuns_icon.png' alt='빼기아이콘'></a></td>";
							text += "<td class='code'>";
							text += "<input type='hidden' id='pdt_cd_"+no+"' name='pdt_cd' value='"+pdt_cd+"'>";
							text +=	"<input type='text' id='pdt_code_"+no+"' name='pdt_code' readonly value='"+pdt_code+"'></td>";
							text += "<td class='prod'><span><input type='text' id='pdt_name_"+no+"' name='pdt_name' value='"+pdt_name+"' placeholder=\"품목\" maxlength='500'></span></td>";
							text += "<td class='prod'><span><input type='text' id='pdt_standard_"+no+"' name='pdt_standard' value='"+pdt_standard+"'  maxlength='100'></span></td>";
							text += "<td class='qty'><input type='text' id='qty_"+no+"' name='qty' value='"+qty+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); priceChange("+no+");' maxlength='10'></td>";
							text += "<td class='name'><input type='text' id='unit_"+no+"' name='unit' value='"+unit+"' maxlength='100'></td>";
							text += "<td class='qty'><input type='text' id='unit_price_"+no+"' name='unit_price' value='"+comma(unit_price)+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); priceChange("+no+");' maxlength='12'></td>";
							text += "<td class='qty'><input type='text' id='supply_price_"+no+"' name='supply_price' value='"+comma(supply_price)+"' readonly></td>";
							text += "<td class='qty'><input type='text' id='vat_"+no+"' name='vat' value='"+comma(vat)+"' readonly></td>";
							text += "</tr>";

	                	}
	                   
	            	}else{
	            		text += "<tr class='all'><td colspan='8'>수주정보가 없습니다.</td></tr>";
	            	}
...
/*
    		...
    		text += "<td class='code'>";
			text += "<input type='hidden' id='pdt_cd_1' name='pdt_cd' value='4099'>";
    		text += "<input type='text' id='pdt_code_1' name='pdt_code' value='ETC'></td>";
    		text += "<td class='prod'>";
    		text += "<div class=\"dan\">";
    		...
    		text += "</div>";
    		text += "</td>";
    		text += "<td class='name'><input type='text' id='qty_1' name='qty'\ onChange='priceChange(1); return false;'></td>";
	...
	function GoActionButton(){
       ...
       if (status == false) return;
		...
		for (var i = 0; i < $("input[name='pdt_name']").length; i++) {
			...
			contractDetailList.push(tmpMap);
		}
		var contract_info =[];
		var tmpMap2 = {};
		...
		tmpMap2.buyer_addr_info = $($("input[name='buyer_addr_info']")).val();
	    ...
	    sendData.contractDetailList = contractDetailList;
	...
	function contractInfoUpdate(){
		...
		if ($('#contract_status').val() == 'CS004'){

			if (isEmpty($('#cancel_reason').val())){
				alert("사유를 입력해주세요.");
				return false;
			}else{
				msg = "정말 취소하시겠습니까?";
        ...
        <!--  관리자  검색시작-->
                    ...
                    <form id="searchForm" action="/contract/contract">
                    ...
                    <div class="srch_all">
                    	...
                    	<div class="sel_wrap sel_wrap1">
                    		<select name="contract_status" class="sel_02">
								<option value="ALL" <c:if test="${search.contract_status eq 'ALL'}">selected</c:if>>전체</option>
							<c:forEach items="${contractStatus}" var="result">
								<option value="${result.code}" <c:if test="${search.contract_status eq result.code}">selected</c:if>>${result.code_nm}</option>
							</c:forEach>
                            </select>
                    	</div>
	                    		...
	                    		<tbody>
	                    			<c:forEach items="${list}" var="list" varStatus="status">
		                    			<tr>
											...
											<td class="day1">${list.close_date}</td>
											<td class="ing">
											<c:forEach items="${contractStatus}" var="result">
												<c:if test="${result.code eq list.contract_status}">${result.code_nm}</c:if>
											</c:forEach>
											</td>
										</tr>
	                    			</c:forEach>
			...
			<div class="pop_bg" onclick="contractInfoLayer_Close();"></div>
				<div class="pop_wrap pop_wrap_01" >
					...
					<form id="contractInfo_Form" name="contractInfo_Form">
						...
						<h3>수주 등록/조회<a class="back_btn" href="#" onclick="contractInfoLayer_Close();"><img src="/images/common/back_arrow.png" alt="뒤로가기"></a></h3>
							...
							<table  class="master_02 master_04">	
								...
								<tbody>
									<tr>
										...
										<td colspan="3">
											<input type="text" id="contract_code" name="contract_code" readonly="readonly"/>
										</td>
									</tr>
									<tr>
										<th scope="row">수주처 <span class="keyf01">*</span></th>
										<td colspan="3">
											<input type="text" id="buyer_name" name="buyer_name" class="all" placeholder="수주처" onclick="openLayer();return false;" readonly />
										</td>
									</tr>
									<tr>
										<th scope="row">담당자  <span class="keyf01">*</span></th>
										<td><input type="text" id="buyer_manager_name" name="buyer_manager_name" maxlength="45" placeholder="담당자"/></td>
										<th scope="row">연락처  <span class="keyf01">*</span></th>
										<td><input type="text" id="buyer_phonenumber" name="buyer_phonenumber" maxlength="20" placeholder="연락처"/></td>
									</tr>

									<tr>
										<th scope="row">수주일자</th>
										<td><input type="text" id="contract_date" name="contract_date" readonly="readonly"/></td>
										<th scope="row">납기일자</th>
										<td><div class="day_input"><input type="text" id="close_date" name="close_date" /></div></td>
									</tr>

									<tr>
										<th scope="row" rowspan="2">주소</th>
										<td colspan="3">
											<input type="text" id="post_no" name="post_no" class="add" readonly placeholder="우편번호"/>
											<button type="button" class="btn_02 btn_s" id="button_3" onclick="openZipSearch();return false;">주소 검색</button>
										</td>
									</tr>
									<tr>
										<td colspan="3">
											<input type="text"  id="buyer_addr" name="buyer_addr" placeholder="주소" class="all" style="margin-bottom: 5px;" readonly/>
											<br><input type="text"  id="buyer_addr_info" name="buyer_addr_info" placeholder="상세주소" class="all" maxlength="45"/>
										</td>
									</tr>

									<tr>
										<th scope="row">수주담당자</th>
										<td><input type="text" id="contract_manager_name" name="contract_manager_name" maxlength="45" readonly="readonly"/></td>
										
										<th scope="row">연락처</th>
										<td><input type="text" id="contract_phonenumber" name="contract_phonenumber" maxlength="20"/></td>
									...
									<tr>	
										...
										<th scope="row">비고</th>										
										<td><input type="text" id="bigo" name="bigo" class="all" maxlength="100"/></td>
									...
									<tr id="sayou" style="display : none;">
										<th scope="row">사유  <span class="keyf01">*</span></th>
										<td colspan="3"><input type="text" id="cancel_reason" name="cancel_reason" class="all" maxlength="100"/></td>
									</tr>				
						...
						<div class="master_list master_listT">
							...
							<div class="scroll">
								<table id="productZone" class="master_01 master_05" id="product" >
									<colgroup>
										<col style="width: 55px;"/>
...
<script>
	...
	function productSearch(){
		if (sessionCheck()){
			var url = "/order/popProductList?type=contract";
	...
	function PopProductAdd(data){
		...
		text += "<td class='prod'><span><input type='text' id='pdt_name_"+num+"' name='pdt_name' value='"+pdt.PDT_NAME+"' placeholder=\"품목\" maxlength='500'></span></td>";
		text += "<td class='prod'><span><input type='text' id='pdt_standard_"+num+"' name='pdt_standard' value='"+pdt.PDT_STANDARD+"' placeholder=\"규격\" maxlength='100'></span></td>";
		text += "<td class='qty'><input type='text' id='qty_"+num+"' name='qty' value='1' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); priceChange("+num+");' maxlength='10'></td>";
		text += "<td class='name'><input type='text' id='unit_"+num+"' name='unit' value='"+pdt.UNIT+"' maxlength='100'></td>";
		text += "<td class='qty'><input type='text' id='unit_price_"+num+"' name='unit_price' value='"+comma(pdt.OUT_PRICE)+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); priceChange("+num+");' maxlength='12'></td>";
		text += "<td class='qty'><input type='text' id='supply_price_"+num+"' name='supply_price' value='"+comma(pdt.OUT_PRICE)+"' readonly></td>";
		text += "<td class='qty'><input type='text' id='vat_"+num+"' name='vat' value='"+comma(Math.round(pdt.OUT_PRICE * 0.1))+"' readonly></td>";
		text += "</tr>";
		...
		if(sessionCheck()){
			new daum.Postcode({
				oncomplete: function(data) {
					...
					$('[name=buyer_addr]').val(data.address);
				}
			}).open();
		}else{
			location.reload();
