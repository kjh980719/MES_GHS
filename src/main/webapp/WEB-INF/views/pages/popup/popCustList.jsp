...
<script>
		...
		var formData = $('#searchPopForm').serialize();
		 $.ajax({
		       ...
		       success : function(data) {
		    	   ...
		    	   if (list.length > 0){
		    		   for (var i in list){
			    		   text += "<tr onclick='choiceCust(\""+list[i].CUST_NAME+"\",\""+list[i].BUSINESS_NO+"\",\""+list[i].CUST_SEQ+"\");'>";
	...
	function choiceCust(name, no, cust_seq){
		console.log(parent.document.getElementById("workplanInfo_Form"));
		if(parent.document.getElementById("workplanInfo_Form") != null){
			parent.document.workplanInfo_Form.cust_seq.value = cust_seq;
			parent.document.workplanInfo_Form.cust_name.value = name;
		}else {
			parent.document.producePlanInfo_Form.cust_seq.value = cust_seq;
			parent.document.producePlanInfo_Form.cust_name.value = name;
		}
		...
		<div class="pop_wrap pop_wrap_02">
			<div class="pop_inner">
				<div class="master_wrap master_layer">
					...
					<form id="searchPopForm" action="/cust/popCustList">
