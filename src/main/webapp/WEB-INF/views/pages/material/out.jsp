...
<div class="master_cont">
	<form id="searchForm" action="/product/in">
		...
		<div class="srch_day">
			<div class="day_area">
				<div class="day_label">
					<label for="startDate">출고일자</label>
				</div>
				<div class="day_input">
		...
		<div class="srch_all">
			...
			<div class="sel_wrap sel_wrap1">
				<select name="search_storage" class="sel_02">
					<option value="0" <c:if test="${search.search_storage eq 0}">selected</c:if>>출고창고</option>
	...
	<table class="master_01">
		...
		<c:forEach items="${list}" var="list" varStatus="status">
			<tr onclick="viewMaterialInDetail('${list.io_seq}','view');">
				...
				<td class="name">${list.product_line}</td>
				<td class="prod">
				...
				<td class="day day1">${list.out_date}</td>
			</tr>
		</c:forEach>
		<c:if test="${empty list }">
			<tr><td colspan="6">자재출고 정보가 없습니다.</td></tr>
		</c:if>
		...
		<div class="pop_wrap pop_wrap_01" >
			<div class="pop_inner">
				<form id="inmaterialInfo_Form" name="inmaterialInfo_Form">
					...
					<h3>자재 출고<a class="back_btn" href="#" onclick="inmaterialInfoLayer_Close();"><img src="/images/common/back_arrow.png" alt="뒤로가기"></a></h3>
					...
					<div class="master_list master_listB">
						<table  class="master_02 master_04">
							...
							<tbody>
							<tr>
								<th scope="row">출고코드 </th>
								<td colspan="3">
									<input type="text" id="io_code" name="io_code" readonly="readonly"/>
									<button id="button_1" type="button" class="btn_02 btn_s" onclick="openLayer();" style="margin-left: 5px;">BOM 불러오기</button>
								</td>
							</tr>
							<tr>
								<th scope="row">출고일자 <span class="keyf01">*</span></th>
								<td colspan="3">
									<div class="day_input"><input type="text" id="out_date" name="out_date" readonly/></div>
			...
			<div id="popup" class="layer_pop">
				<div class="handle_wrap">
					<div class="handle"><span>BOM 리스트</span></div>
...
<script type="text/javascript">
	...
	function productSearch() {
		if (sessionCheck()) {
			var url = "/order/popProductList?type=mOut";
				...
				success : function(data) {
					...
					if (array.length > 0) {
						...
						for (var i in array){
							...
							text += "<td class='qty'><input type='text' id='qty_" + array[i].no + "' name='qty' value='"+comma(array[i].qty)+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); this.value=comma(this.value); priceChange(" + array[i].no + ");' maxlength='12' placeholder='출고수량'></td>";
						...
						text += "<tr class='all'><td colspan='9'>출고정보가 없습니다.</td></tr>";
					}
	...
	function GoActionButton(){
		...
		var mode = $('#mode').val();
		if (mode == 'register') {
			targetUrl = "/material/registerOut";
		} else {
			targetUrl = "/material/updateOut";
		}
		...
		tmpMap2.out_date = $($("input[name='out_date']")).val();
	...
	function openLayer() {
		if (sessionCheck()){
			var url = "/order/popProductList?type=out";
	...
	function PopPartListAdd(data){
		...
		for (var i in array){
			...
			text += "<input type='text' id='pdt_code_" + num + "' name='pdt_code' readonly value='" + array[i].MTR_CODE + "'></td>";
			...
			text += "<td class='qty'><input type='text' id='qty_" + num + "' name='qty' value='"+comma(array[i].QTY)+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); this.value=comma(this.value); priceChange(" + num + ");' maxlength='12' placeholder='출고수량'></td>";
			text += "<td><div class='sel_wrap sel_wrap1'><select name='STORAGE_NAME1' id='STORAGE_NAME1_"+num+"' onchange='categoryChange(this)'class='sel_02'>";
			text += StorageList(0,1);
			text +="</select></div></td>";
			text += "<td><div class='sel_wrap sel_wrap1'><select name='STORAGE_NAME2' id='STORAGE_NAME2_"+num+"'onchange='categoryChange(this)' class='sel_02'>";
	...
	function PopWorkPlanAdd(data){
		...
		for (var i in array){
			...
			text += "<td class='name'><input type='text' id='unit_" + num + "' name='unit' value='" + array[i].unit + "'></td>";
			text += "<td class='qty'><input type='text' id='qty_" + num + "' name='qty' value='"+comma(array[i].planqty)+"' onkeyup='this.value=this.value.replace(/[^0-9]/g,\"\"); this.value=comma(this.value); priceChange(" + num + ");' maxlength='12' placeholder='출고수량'></td>";
			text += "<td><div class='sel_wrap sel_wrap1'><select name='STORAGE_NAME1' id='STORAGE_NAME1_"+num+"' onchange='categoryChange(this)'class='sel_02'>";
			text += StorageList(0,1);
			text +="</select></div></td>";
			text += "<td><div class='sel_wrap sel_wrap1'><select name='STORAGE_NAME2' id='STORAGE_NAME2_"+num+"'onchange='categoryChange(this)' class='sel_02'>";
