...
$(document).ready(function() {
	...
	var checkProductValid = function () {
        ...
        if(valid) {
             if($('#ProductRegistForm input[name="PROD_CD"]').val() == "") {
                ...
                /* if($("#ProductRegistForm input[name='newPassword']").val() != $("#ProductRegistForm input[name='newPasswordConfirm']").val()) {
                    alert("새 비밀번호 확인 이 새 비밀번호와 다릅니다.");
                    $("#ProductRegistForm input[name='newPasswordConfirm']").focus();
                    return false;
                } */
            }else{
            	
            } /* else if($('#ProductRegistForm input[name="managerSeq"]').val() == "") {
                //신규 등록
                if($("#ProductRegistForm input[name='managerId']").val().length > 20) {
                    alert("관리자 아이디는 최대 20자 입니다.");
                    $("#ProductRegistForm input[name='managerId']").focus();
                    return false;
                }
                if(!checkPassword($("#ProductRegistForm input[name='password']").val())) {
                    alert("비밀번호는 영문(1자이상), 숫자(1자이상)을 포함하는 8~16자를 입력해 주십시오.");
                    $("#ProductRegistForm input[name='password']").focus();
                    return false;
                }
            }  */
	...
	setCalendarDefault();
    var colModel = [
        {label: "번호", name:"rowIndex", align: "center", width: "10%", sortable: false},
        {label: "분류", name:"PROD_CD", align: "center", width: "10%", sortable: false},
        {label: "제목", name:"PROD_DES", align: "center", width: "10%", sortable: false},
        {label: "전시상태", name:"PROD_TYPE", align: "center", width: "10%", sortable: false},
        {label: "전시시작일", name:"SIZE_DES", align: "center", width: "10%", sortable: false},
        {label: "전시종료일", name:"CAT_CD", align: "center", width: "10%", sortable: false},
        {label: "등록일", name:"REMARKS_WIN", align: "center", width: "10%", sortable: false}
    ];
		...
		.tab-content.current{
			display: inherit;
					...
					<div class="align_rgt mjinput">
						<div class="mjRight">
							<button type="button" class="btn_blue01 popLayerproductRegistBtn">등록</button>
						</div>
					</div>
        <!--  관리자  검색시작-->
