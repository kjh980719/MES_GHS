...
<div class="master_cont">
	<form id="searchForm" action="/product/out">
		...
		<div class="srch_all">
			...
			<div class="sel_wrap sel_wrap1">
				<select name="search_type" class="sel_02">
					...
					<option value="IO_CODE" <c:if test="${search.search_type == 'IO_CODE'}">selected</c:if>>출고코드</option>
				</select>
			</div>
...
<div class="scroll">
	<table class="master_01">
		...
		<c:if test="${empty list }">
			<tr><td colspan="6">출고 정보가 없습니다.</td></tr>
		</c:if>
		...
		<div class="pop_wrap pop_wrap_01" >
			<div class="pop_inner">
				<form id="inmaterialInfo_Form" name="inmaterialInfo_Form">
					...
					<h3>출고관리<a class="back_btn" href="#" onclick="inmaterialInfoLayer_Close();"><img src="/images/common/back_arrow.png" alt="뒤로가기"></a></h3>
						...
						<table  class="master_02 master_04">
							...
							<tr>
								...
								<td colspan="3">
									...
									<input type="text" id="allFrom_txt_1" placeholder="From 일괄적용" onclick="choiceLocation_open(this);">
									<input type="hidden" id="allTo_type_1"/>
									<input type="hidden" id="allTo_location_1"/>
									<input type="text" id="allTo_txt_1" placeholder="To 일괄적용" onclick="choiceLocation_open(this);">
								</td>
							</tr>
							</tbody>
						</table>
...
<script type="text/javascript">
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (array.length > 0) {
						...
						text += "<tr class='all'><td colspan='11'>출고정보가 없습니다.</td></tr>";
					}
	...
	function GoActionButton(){
		...
		var mode = $('#mode').val();
		if (mode == 'register') {
			targetUrl = "/product/registerOut";
		} else {
			targetUrl = "/product/updateOut";
		}
