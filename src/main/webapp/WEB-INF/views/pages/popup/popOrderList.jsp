...
<script>
		 ...
		 $.ajax({
		       ...
		       success : function(data) {
				   ...
				   if (list.length > 0){
					   total = list[0].total;
		    		   for (var i in list){
			    		   text += "<tr onclick='choiceorder(\""+list[i].order_seq+"\")'>";
			    		   text += "<td class='num'>"+list[i].no+"</td>";
			    		   text += "<td class='code' >"+list[i].order_code+"</td>";
			    		   text += "<td class='sang t_left'>"+list[i].pdt_name+"</td>";
			    		   text += "<td class='name'>"+ list[i].supply_name+"</td>";
			    		   text += "<td class='name'>"+ list[i].order_date+"</td>";
						   text += "<td class='ing'>" + ((list[i].order_status == 'OS004') ? '진행중' : '완료') + "</td>";
			    		   text += "</tr>";
			    	   }
		    	   }else{
		    		   text ="<tr><td colspan='6'>발주 정보가 없습니다.</td></tr>";
		    	   } 
	...
	function choiceorder(ORDER_SEQ){
	
		$.ajax({
		       type : "get",            
		       url : '/order/getPopOrderInfo',            
		...
		<div class="pop_wrap pop_wrap_02">
			<div class="pop_inner">
				<div class="master_wrap master_layer">
					<h3>발주 리스트<a class="back_btn" href="#" onclick="order_close();"><img src="/images/common/back_arrow.png" alt="뒤로가기"></a></h3>
						...
						<div class="srch_all">
							<div class="sel_wrap sel_wrap1">
								<select name="search_type" class="sel_02">
									...
									<option value="ORDER_CODE">발주번호</option>
