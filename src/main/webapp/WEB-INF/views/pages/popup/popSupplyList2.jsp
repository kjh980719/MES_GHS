...
<script>
	...
	function choiceSupply(name, no, cust_seq){
		...
		if ("${param.type}" == 'notice'){
	 		parent.document.noticeInfo_Form.cust_name.value = name;
			parent.document.noticeInfo_Form.cust_seq.value = cust_seq;
		}else if("${param.type}" == 'inmaterial'){
	 		parent.document.inmaterialInfo_Form.SUPPLY_NAME.value = name;
			parent.document.inmaterialInfo_Form.BUSINESS_NO.value = no;
		}else{
	 		parent.document.contractInfo_Form.buyer_name.value = name;
			parent.document.contractInfo_Form.cust_seq.value = cust_seq;
		}
		layer_close(); 
	}
