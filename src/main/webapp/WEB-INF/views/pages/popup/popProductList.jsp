...
<script>
	...
	$(document).ready(function() {
		...
		if ("${param.type}" == "order" || "${param.type}" == "partlist" || "${param.type}" == "mOut"){
			$('#PROD_TYPE').val("PG001");
		}else{
			$('#PROD_TYPE').val("PG003");
		}
	...
	function choiceProduct(pdt_cd){
		if("${param.type}" == "orderplan" || "${param.type}" == "out"){
			//파트리스트 가져오기
			$.ajax({
				type : "get",
				url : '/orderPlan/getPartListInfo',
				data:  "pdt_cd="+pdt_cd,
				success : function(data) {
					parent.PopPartListAdd(data);
				},beforeSend : function(){
					$('.wrap-loading').removeClass('display-none');
				},error : function(data){
					$('.wrap-loading').addClass('display-none');
			...
			$.ajax({
				...
				data:  "PDT_CD="+pdt_cd,
				success : function(data) {
					if ("${param.type}" == "workplan") {
						var result = parent.pdtCheck(data.storeData);
						if (result == 'N'){
							alert("이미 선택된 품목입니다.");
						}else{
								...
								<div class="sel_wrap sel_wrap1">
									<select id="PROD_TYPE" name="PROD_TYPE" class="sel_02">
										...
										<c:forEach items="${productGubun}" var="result">
											<c:choose>
												<c:when test="${param.type ne 'order' and param.type ne 'partlist' and param.type ne 'mOut'}">
