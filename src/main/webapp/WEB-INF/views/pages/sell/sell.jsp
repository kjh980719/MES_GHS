...
/*        var frm = document.frmWrite;
         if(typeof (frm.pdt_name.length) == "undefined"){
           if(frm.cate_code.value == ""){alert("카테고리를 지정해주세요.");frm.cate_code.focus();return false;}
           if(frm.pdt_name.value == ""){alert("상품명을 입력해주세요.");frm.pdt_name.focus();return false;}
//            if(frm.pdt_spec.value == ""){alert("규격을 입력해주세요.");frm.pdt_spec.focus();return false;}
//            if(frm.pdt_unit.value == ""){alert("단위를 입력해주세요.");frm.pdt_unit.focus();return false;}
        } else{
           for (i = 0; i < frm.pdt_name.length; i++) {
              if(frm.cate_code[i].value == ""){alert("카테고리를 지정해주세요.");frm.cate_code[i].focus();return false;}
              if(frm.pdt_name[i].value == ""){alert("상품명을 입력해주세요.");frm.pdt_name[i].focus();return false;}
//               if(frm.pdt_spec[i].value == ""){alert("규격을 입력해주세요.");frm.pdt_spec[i].focus();return false;}
//               if(frm.pdt_unit[i].value == ""){alert("단위를 입력해주세요.");frm.pdt_unit[i].focus();return false;}
           }
        } */
       ...
       /*  $.ajax({
			...
			if (response.success) {
				...
				$("#OrderMaterialForm input[name='"+response.code+"']").focus();
			}
			$(".registOrderBtn").on("click", function(){registOrder();});
		}); */
     }
	...
	var registOrder = function () {
		...
		/* if(checkOrderValid() && confirm("작성 하시겠습니까?")){
			$(".registOrderBtn").off("click");
			var param = $("#OrderMaterialForm").serializeObject();
			$.ajax({
				type : "post",
				url : "/Order/createOrderMaterial.json",
				contentType: "application/json",
				dataType : "json",
				async : false,
				data: JSON.stringify(param)
